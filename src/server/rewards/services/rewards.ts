...
function observeMilestone(id: string) {
  ...
  const unsubscribeRanking = store.subscribe(
    selectMilestoneRanking(id),
    (ranking = 0) => {
  ...
  const unsubscribeScore = store.subscribe(selectMilestoneScore(id), (score) => {
    ...
    if (reward !== undefined && shouldGrantReward()) {
      grantMoneyReward(
        id,
        reward,
        `hitting a score of <font color="#fff">${score}</font>`,
      )
    }
  })
  ...
  const unsubscribeKill = store.observeWhile(selectMilestoneLastKilled(id), (enemyId) => {
    ...
    if (enemy && shouldGrantReward()) {
      ...
      grantMoneyReward(
        id,
        bounty,
        `eliminating <font color="#fff">${enemy.name}</font>`,
        true,
      )
    }
      ...
      return setInterval(() => {
        ...
        if (reward !== undefined && shouldGrantReward()) {
          grantMoneyReward(
            id,
            reward,
            `staying in the <font color="#fff">top ${rank}</font>`,
          )
        }
      }, 180)
    },
