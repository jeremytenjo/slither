...
export const selectPlayerSnakesById = createSelector(selectSnakesById, (snakesById) => {
  return mapProperties(snakesById, (snake) =>
    getPlayerByName(snake.id) ? snake : undefined,
  )
})
...
export const selectSnakesSorted = (
  comparator: (current: SnakeEntity, existing: SnakeEntity) => boolean,
) => {
